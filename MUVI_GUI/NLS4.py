...
class NewmarkLinearStage:
    ...
    def set_feedback(self, encoder, limit):
        ...
        if self.ZEROED:
            self.enc_pos = self.pos_dir*float(encoder)*self.pitch/self.enc_CPR - self.datum + self.encoder_restore
        else:
            self.enc_pos = self.pos_dir*float(encoder)*self.pitch/self.enc_CPR + self.encoder_restore
