...
class PrintTask:
    def __init__(self, print_q, print_task):
        self.print_queue = print_q
        self.print_task = print_task
        ## The size of the buffer which will hold characters to be printed when the
        #  print task has time to print them.
        # self.BUF_SIZE = const (100)
    ...
    def put (self, a_string):
        ...
        for a_ch in a_string:
            if not self.print_queue.full ():
                # self.print_queue.put (ord (a_ch))
                self.print_queue.put (a_ch.encode('UTF-8'))
                self.print_task.go ()


    #@micropython.native
    def put_bytes (self, b_arr):
    ...
    def run (self):
        ...
        while True:
            ...
            if self.print_queue.any ():
                # print (chr (self.print_queue.get ()), end = '')
                print(self.print_queue.get (),end = '')
